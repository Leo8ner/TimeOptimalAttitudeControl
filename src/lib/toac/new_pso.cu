...
__host__ __device__ my_real fit(my_real *solution_vector, int particle_id, attitude_params *params)
{
    my_real dt = solution_vector[PARTICLE_POS_IDX(particle_id, DT_IDX)];
    ...
    {
        initial_signs[axis] = solution_vector[PARTICLE_POS_IDX(particle_id, axis)];
        // my_real sign_val = solution_vector[PARTICLE_POS_IDX(particle_id, axis)];
                     ...
                     particle_gbest *gbest_d, my_real *aux, my_real *aux_pos)
{
    ...
    for (int dim = 0; dim < dimensions_d; dim++)
    {
        ...
        my_real gbest_pos = gbest_d->position[dim];
    ...
    my_real new_fitness = fit(position_d, particle_id, &att_params_d);
    ...
    : configured_(false), results_valid_(false), max_iterations_(ITERATIONS), num_particles_(N_PARTICLES), inertia_weight_(W), cognitive_weight_(C_1), social_weight_(C_2), particles_(nullptr), position_d_(nullptr), velocity_d_(nullptr), fitness_d_(nullptr), pbest_pos_d_(nullptr), pbest_fit_d_(nullptr), gbest_d_(nullptr), aux_(nullptr), aux_pos_(nullptr), lhs_samples_(nullptr), lhs_generated_(false), verbose_(verbose), X(state_matrix), U(input_matrix), dt(dt_matrix)
{
    ...
    {
        lhs_samples_[i] = new my_real[DIMENSIONS];
    }
    ...
    particles_->position = (my_real *)malloc(sizeof(my_real) * num_particles_ * DIMENSIONS);
    particles_->velocity = (my_real *)malloc(sizeof(my_real) * num_particles_ * DIMENSIONS);
    particles_->pbest_pos = (my_real *)malloc(sizeof(my_real) * num_particles_ * DIMENSIONS);
...
{
    size_t particle_data_size = sizeof(my_real) * num_particles_ * DIMENSIONS;
...
bool PSOOptimizer::extractResults()
{
    ...
    for (int axis = 0; axis < n_controls; axis++)
    {
        my_real sign_val = gbest_.position[axis];
        initial_signs[axis] = (sign_val >= REAL(0.0)) ? REAL(1.0) : REAL(-1.0);
    }
