...
// Global DynamicsIntegrator instance (eager initialization)
...
// Build sparse patterns from DynamicsIntegrator
...
static int casadi_f0(const casadi_real** arg, casadi_real** res, casadi_int* iw, casadi_real* w, int mem) {
  ...
  int ret = g_integrator->solve(arg[0], arg[1], arg[2], false);  // No sensitivity for forward
  if (ret != 0) return ret;
...
//   // Call CUDA integrator with sensitivity
//   int ret = g_integrator->solve(arg[0], arg[1], arg[2], true);  // Enable sensitivity
//   if (ret != 0) return ret;
  
//   // Get sensitivities
//   ret = g_integrator->getSensitivitiesY0(res[0]);  // dy/dy0
//   if (ret != 0) return ret;
  
//   ret = g_integrator->getSensitivitiesU(res[1]);   // dy/du
//   if (ret != 0) return ret;

//   jacobian_cnt++; // Increment Jacobian call count
//   //printf("Jacobian call count: %d\n", jacobian_cnt);

//   return g_integrator->getSensitivitiesDt(res[2]); // dy/dt
// }
...
static int casadi_f1(const casadi_real** arg, casadi_real** res, casadi_int* iw, casadi_real* w, int mem) {
  ...
  const sunrealtype epsilon = 1e-6;  // Finite difference step size
  
  // arg[3] is the baseline solution - no need to recompute it
  const sunrealtype* baseline_solution = arg[3];
  
  // Allocate arrays for perturbed inputs and solutions
  sunrealtype perturbed_states[n_states_total];
  sunrealtype perturbed_torques[n_controls_total];
  sunrealtype perturbed_dt[n_stp];
  sunrealtype perturbed_solution[n_states_total];
  ...
  //==========================================================================
  // Step 1: Initial condition sensitivities (7 calls)
  //==========================================================================
  
  for (int state_idx = 0; state_idx < n_states; state_idx++) {
    
    // Copy baseline and perturb this state for all systems simultaneously
    memcpy(perturbed_states, arg[0], n_states_total * sizeof(sunrealtype));

    for (int sys = 0; sys < n_stp; sys++) {
      perturbed_states[sys * n_states + state_idx] += epsilon;
    }
    
    int result = g_integrator->solve(perturbed_states, arg[1], arg[2], false);
    if (result != 0) {
      printf("Error: Perturbed solve failed for state %d with code %d\n", state_idx, result);
      return -1;
    }
    
    g_integrator->getSolution(perturbed_solution);
    
    // Compute finite differences following CasADi sparsity pattern
    y0_value_idx = (state_idx < n_quat ? state_idx * n_quat : n_quat * n_quat + (state_idx - n_quat) * n_states);
    for (int sys = 0; sys < n_stp; sys++) {
      if (state_idx < n_quat) {
        // Quaternion state: affects only quaternions of same system (4 entries)
        for (int quat_comp = 0; quat_comp < n_quat; quat_comp++) {
          int row = sys * n_states + quat_comp;
          sunrealtype fd_grad = (perturbed_solution[row] - baseline_solution[row]) / epsilon;
          res[0][y0_value_idx++] = fd_grad;
        }
        y0_value_idx += nnz - n_quat;
      } else {
  ...
  //==========================================================================
  // Step 2: Control parameter sensitivities (3 calls)
  //==========================================================================
  ...
  for (int torque_idx = 0; torque_idx < n_controls; torque_idx++) {
    ...
    memcpy(perturbed_torques, arg[1], n_stp * n_controls * sizeof(sunrealtype));
    
    for (int sys = 0; sys < n_stp; sys++) {
      perturbed_torques[sys * n_controls + torque_idx] += epsilon;
    }
    
    int result = g_integrator->solve(arg[0], perturbed_torques, arg[2], false);
    if (result != 0) {
      printf("Error: Perturbed solve failed for torque %d with code %d\n", torque_idx, result);
      return -1;
    }
  ...
  //==========================================================================
  // Step 3: Time step sensitivities (1 call)
  //==========================================================================
  
  // Copy baseline and perturb dt for all systems simultaneously
  memcpy(perturbed_dt, arg[2], n_stp * sizeof(sunrealtype));
  
  for (int sys = 0; sys < n_stp; sys++) {
    perturbed_dt[sys] += epsilon;
  }
  
  int result = g_integrator->solve(arg[0], arg[1], perturbed_dt, false);
  if (result != 0) {
    printf("Error: Perturbed solve failed for dt with code %d\n", result);
    return -1;
  }
