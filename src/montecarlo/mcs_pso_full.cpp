...
int main() {
    ...
    PSOOptimizer initial_guess(X_guess, U_guess, dt_guess, PSOMethod::FULL, false); // Create PSO optimizer instance
    Function solver = get_solver();

    // Open CSV file for logging results
    std::ofstream results_file("../output/mcs_pso_full.csv");
    ...
    results_file.close();
    std::cout << "Results logged to output/mcs_pso_full.csv" << std::endl;
