...
/** @brief Maximum allowed switches per torque control axis */
#define MAX_SWITCHES_PER_AXIS 3  // Adjust based on problem

/** @brief Total number of switch times for all axes */
#define N_SWITCH_TIMES (n_controls * MAX_SWITCHES_PER_AXIS)  // Switch times for all axes

/** @brief Total optimization dimensions: torque variables + time step */
#define DIMENSIONS (n_controls + n_controls*MAX_SWITCHES_PER_AXIS + 1) 
 ...
 * @param switch_num Switch index [0, MAX_SWITCHES_PER_AXIS-1]
 * @param axis Control axis index [0, n_controls-1] (typically 0=x, 1=y, 2=z)
 * @return Dimension index for torque variable
 */
#define SWITCH_TIME_IDX(axis, switch_num) (n_controls + (axis)*MAX_SWITCHES_PER_AXIS + (switch_num))
