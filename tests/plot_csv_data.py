...
def main(csv_path: str | Path) -> None:
    ...
    plt.figure()
    plt.plot(time, X[0], label=r"$q_0$")
    plt.plot(time, X[1], label=r"$q_1$")
    plt.plot(time, X[2], label=r"$q_2$")
    plt.plot(time, X[3], label=r"$q_3$")
    plt.xlabel("Time  [s]")
    plt.ylabel("Quaternions [-]")
    plt.title("Quaternions with time")
    plt.grid(True)
    plt.legend()
    plt.savefig("quaternions.pdf" , format='pdf', dpi=600, bbox_inches='tight')


    plt.figure()
    plt.plot(time, X[4]*180/np.pi, label=r'$\omega_x$')
    plt.plot(time, X[5]*180/np.pi, label=r"$\omega_y$")
    plt.plot(time, X[6]*180/np.pi, label=r"$\omega_z$")
    plt.xlabel("Time  [s]")
    plt.ylabel("Angular Rates [deg/s]")
    plt.title("Angular rates with time")
    plt.grid(True)
    plt.legend()
    plt.savefig("angular_rates.pdf" , format='pdf', dpi=600, bbox_inches='tight')
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig("control_inputs.pdf" , format='pdf', dpi=600, bbox_inches='tight')
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python3 plot_csv_data.py trajectory.csv")
        sys.exit(1)
    main(sys.argv[1])
