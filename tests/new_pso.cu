...
int main(int argc, char** argv) {
    ...
    DM X(n_states, (n_stp + 1)), U(n_controls, n_stp), dt(n_stp, 1); // Initial guesses for states, controls, and time steps
    
    std::cout << "=== PSO Bang-Bang Attitude Control Test ===" << std::endl;
    std::cout << "Problem dimensions: " << DIMENSIONS << " parameters" << std::endl;
    std::cout << "  - " << n_controls << " initial control signs" << std::endl;
    std::cout << "  - " << N_SWITCH_TIMES << " switch times" << std::endl;
    std::cout << "  - 1 timestep parameter" << std::endl;
    std::cout << "Maximum switches per axis: " << MAX_SWITCHES_PER_AXIS << std::endl;
    
    // Create optimizer (verbose mode enabled)
    PSOOptimizer optimizer(X, U, dt, true);
    
    // Set initial and target states
    optimizer.setStates(X_0->data(), X_f->data());
    
    // Run optimization
    bool success = optimizer.optimize(true);
    
    if (success) {
        // Get statistics
        DMDict PSOresults = {{"X", X}, {"U", U}, {"T", sum(dt)}, {"dt", dt}};
