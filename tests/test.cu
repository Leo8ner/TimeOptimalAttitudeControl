...
__global__ void sensitivityRHS(int Ns, sunrealtype* y, sunrealtype** yS_data_array, 
    ...
    // Add direct parameter dependencies: ∂f/∂u terms
...
int DynamicsIntegrator::sensitivityRHSFunction(int Ns, sunrealtype t, N_Vector y, N_Vector ydot,
    ...
    sensitivityRHS<<<sensGridSize, sensBlockSize, 0, integrator->compute_stream>>>(
        Ns, y_data, integrator->d_yS_ptrs, integrator->d_ySdot_ptrs, integrator->d_torque_params_ptr,
        integrator->d_dt_params_ptr);
...
/**
 * @brief Compute complete sensitivity analysis using finite differences
 * 
 * Efficiently computes all sensitivities ∂y/∂y₀, ∂y/∂u, and ∂y/∂dt using only
 * 11 solve calls by leveraging system independence. Each call perturbs the same
 * parameter type across all systems simultaneously.
 * 
 * Call sequence:
 * 1. Baseline solve (1 call)
 * 2. Perturb each initial condition type (7 calls: q0,q1,q2,q3,wx,wy,wz)  
 * 3. Perturb each torque component (3 calls: τx,τy,τz)
 * 4. Perturb time step (1 call: dt)
 * Total: 12 calls
 * 
 * @param initial_states Baseline initial conditions [n_stp × n_states]
 * @param torque_params Baseline torque parameters [n_stp × n_controls] 
 * @param delta_t Baseline time steps [n_stp]
 * @param epsilon Finite difference step size (typically 1e-6 to 1e-8)
 * @param y0_sens_values Output array for ∂y/∂y₀ values [y0_nnz]
 * @param u_sens_values Output array for ∂y/∂u values [u_nnz]
 * @param dt_sens_values Output array for ∂y/∂dt values [dt_nnz]
 * @return 0 on success, negative error code on failure
 */
int DynamicsIntegrator::computeFullSensitivityFiniteDifference(
    const sunrealtype* initial_states,
    const sunrealtype* torque_params, 
    const sunrealtype* delta_t,
    sunrealtype epsilon,
    sunrealtype* y0_sens_values,
    sunrealtype* u_sens_values, 
    sunrealtype* dt_sens_values) {
    
    // Validate inputs
    if (!y0_sens_values || !u_sens_values || !dt_sens_values) {
        cerr << "Error: Output arrays not allocated" << endl;
        return -1;
    }
    
    // Ensure sparsity patterns are computed
    if (!sens_was_setup) {
        cerr << "Error: Sensitivity analysis not setup. Call setupSensitivityAnalysis() first." << endl;
        return -1;
    }
    
    cout << "\n=== Full Finite Difference Sensitivity Analysis ===" << endl;
    cout << "Epsilon: " << scientific << epsilon << endl;
    cout << "Expected solve calls: 12 (1 baseline + 7 states + 3 torques + 1 dt)" << endl;
    
    //==========================================================================
    // Step 1: Baseline solution
    //==========================================================================
    
    cout << "Computing baseline solution..." << endl;
    int result = solve(initial_states, torque_params, delta_t, false);
    if (result != 0) {
        cerr << "Error: Baseline solve failed with code " << result << endl;
        return -1;
    }
    ...
    // Allocate arrays for perturbed inputs and solutions
    sunrealtype perturbed_states[n_total];
    sunrealtype perturbed_torques[n_stp * n_controls];
    sunrealtype perturbed_dt[n_stp];
    sunrealtype perturbed_solution[n_total];
    
    // Index counters for filling output arrays
    int y0_value_idx = 0;
    int u_value_idx = 0;
    int dt_value_idx = 0;
    
    //==========================================================================
    // Step 2: Initial condition sensitivities (7 calls)
    //==========================================================================
    
    cout << "Computing ∂y/∂y₀ sensitivities..." << endl;
    
    for (int state_idx = 0; state_idx < n_states; state_idx++) {
        cout << "  Perturbing state " << state_idx 
             << " (" << (state_idx < 4 ? "quaternion" : "angular velocity") << ")" << endl;
        
        // Copy baseline and perturb this state for all systems simultaneously
        memcpy(perturbed_states, initial_states, n_total * sizeof(sunrealtype));
        
        for (int sys = 0; sys < n_stp; sys++) {
            perturbed_states[sys * n_states + state_idx] += epsilon;
        }
        
        result = solve(perturbed_states, torque_params, delta_t, false);
        if (result != 0) {
            cerr << "Error: Perturbed solve failed for state " << state_idx 
                 << " with code " << result << endl;
            return -1;
        }
        
        getSolution(perturbed_solution);
        
        // Compute finite differences following sparsity pattern

        y0_value_idx = (state_idx < 4 ? state_idx * 4 : 16 + (state_idx - 4) * 7);
        for (int sys = 0; sys < n_stp; sys++) {
            if (state_idx < 4) {
                // Quaternion state: affects only quaternions of same system (4 entries)
                for (int quat_comp = 0; quat_comp < 4; quat_comp++) {
                    int row = sys * n_states + quat_comp;
                    sunrealtype fd_grad = (perturbed_solution[row] - baseline_solution[row]) / epsilon;
                    y0_sens_values[y0_value_idx++] = fd_grad;
                }
                y0_value_idx += nnz - 4;
            } else {
                // Angular velocity state: affects all states of same system (7 entries)
                for (int state_comp = 0; state_comp < n_states; state_comp++) {
                    int row = sys * n_states + state_comp;
                    sunrealtype fd_grad = (perturbed_solution[row] - baseline_solution[row]) / epsilon;
                    y0_sens_values[y0_value_idx++] = fd_grad;
                    
                }
                y0_value_idx += nnz - n_states;
            }
        }
    }

    y0_value_idx -= nnz - n_states; // Adjust for initial conditions
    
    cout << "  ∂y/∂y₀ computed: " << y0_value_idx << " values" << endl;
    
    //==========================================================================
    // Step 3: Control parameter sensitivities (3 calls)
    //==========================================================================
    
    cout << "Computing ∂y/∂u sensitivities..." << endl;
    for (int torque_idx = 0; torque_idx < n_controls; torque_idx++) {
        cout << "  Perturbing torque " << torque_idx 
             << " (τ" << (torque_idx == 0 ? "x" : torque_idx == 1 ? "y" : "z") << ")" << endl;
        
        // Copy baseline and perturb this torque for all systems simultaneously
        memcpy(perturbed_torques, torque_params, n_stp * n_controls * sizeof(sunrealtype));
        
        for (int sys = 0; sys < n_stp; sys++) {
            perturbed_torques[sys * n_controls + torque_idx] += epsilon;
        }
        
        result = solve(initial_states, perturbed_torques, delta_t, false);
        if (result != 0) {
            cerr << "Error: Perturbed solve failed for torque " << torque_idx 
                 << " with code " << result << endl;
            return -1;
        }
        ...
        // Each torque affects all states of same system (dense coupling)
        u_value_idx = n_states * torque_idx;
        for (int sys = 0; sys < n_stp; sys++) {
            for (int state_comp = 0; state_comp < n_states; state_comp++) {
                int row = sys * n_states + state_comp;
                sunrealtype fd_grad = (perturbed_solution[row] - baseline_solution[row]) / epsilon;
                u_sens_values[u_value_idx++] = fd_grad;
            }
            u_value_idx += n_controls * n_states - n_states; // Skip to next torque
        }
    }
    u_value_idx -= n_controls * n_states - n_states; // Adjust for initial conditions
    cout << "  ∂y/∂u computed: " << u_value_idx << " values" << endl;
    
    //==========================================================================
    // Step 4: Time step sensitivities (1 call)
    //==========================================================================
    
    cout << "Computing ∂y/∂dt sensitivities..." << endl;
    
    // Copy baseline and perturb dt for all systems simultaneously
    memcpy(perturbed_dt, delta_t, n_stp * sizeof(sunrealtype));
    
    for (int sys = 0; sys < n_stp; sys++) {
        perturbed_dt[sys] += epsilon;
    }
    
    result = solve(initial_states, torque_params, perturbed_dt, false);
    if (result != 0) {
        cerr << "Error: Perturbed solve failed for dt with code " << result << endl;
        return -1;
    }
    
    getSolution(perturbed_solution);
    
    // Each dt affects all states of same system (dense coupling)
    for (int sys = 0; sys < n_stp; sys++) {
        for (int state_comp = 0; state_comp < n_states; state_comp++) {
            int row = sys * n_states + state_comp;
            sunrealtype fd_grad = (perturbed_solution[row] - baseline_solution[row]) / epsilon;
            dt_sens_values[dt_value_idx++] = fd_grad;
        }
    }
    
    cout << "  ∂y/∂dt computed: " << dt_value_idx << " values" << endl;
    
    //==========================================================================
    // Validation and summary
    //==========================================================================
    
    // Verify we filled arrays correctly
    int expected_y0 = getSparsitySizeY0();
    int expected_u = getSparsitySizeU(); 
    int expected_dt = getSparsitySizeDt();
    
    if (y0_value_idx != expected_y0) {
        cerr << "Error: ∂y/∂y₀ size mismatch. Expected " << expected_y0 
             << ", got " << y0_value_idx << endl;
        return -1;
    }
    
    if (u_value_idx != expected_u) {
        cerr << "Error: ∂y/∂u size mismatch. Expected " << expected_u 
             << ", got " << u_value_idx << endl;
        return -1;
    }
    
    if (dt_value_idx != expected_dt) {
        cerr << "Error: ∂y/∂dt size mismatch. Expected " << expected_dt 
             << ", got " << dt_value_idx << endl;
        return -1;
    }
    
    cout << "Finite difference sensitivity analysis completed successfully!" << endl;
    cout << "Total solve calls: 12 (as expected)" << endl;
    cout << "Computed sensitivities: " << (expected_y0 + expected_u + expected_dt) << " values" << endl;
    
    return 0;
}

/**
 * @brief Compare analytical vs finite difference sensitivities
 * 
 * Comprehensive validation method that computes both analytical and finite
 * difference sensitivities, then compares them statistically.
 * 
 * @param initial_states Test initial conditions
 * @param torque_params Test torque parameters  
 * @param delta_t Test time steps
 * @param epsilon Finite difference step size
 * @param tolerance Maximum acceptable error for pass/fail
 * @return 0 if comparison passes, negative if fails
 */
int DynamicsIntegrator::compareAnalyticalVsFiniteDifference(
    sunrealtype epsilon,
    ...
    cout << "\n=== Analytical vs Finite Difference Comparison ===" << endl;
    
    if (!sens_was_setup) {
        cerr << "Error: Sensitivity analysis not enabled" << endl;
        return -1;
    }
    
    // Get sensitivity array sizes
    int y0_nnz = getSparsitySizeY0();
    int u_nnz = getSparsitySizeU();
    int dt_nnz = getSparsitySizeDt();
    
    if (y0_nnz < 0 || u_nnz < 0 || dt_nnz < 0) {
        cerr << "Error: Failed to get sensitivity sizes" << endl;
        return -1;
    }
    
    // Allocate arrays
    sunrealtype* analytical_y0 = new sunrealtype[y0_nnz];
    sunrealtype* analytical_u = new sunrealtype[u_nnz];
    sunrealtype* analytical_dt = new sunrealtype[dt_nnz];
    
    sunrealtype* fd_y0 = new sunrealtype[y0_nnz];
    sunrealtype* fd_u = new sunrealtype[u_nnz];
    sunrealtype* fd_dt = new sunrealtype[dt_nnz];
    
    try {
        // Compute analytical sensitivities
        cout << "Computing analytical sensitivities..." << endl;
        int result = solve(initial_states, torque_params, delta_t, true);
        if (result != 0) {
            cerr << "Error: Analytical solve failed" << endl;
            delete[] analytical_y0; delete[] analytical_u; delete[] analytical_dt;
            delete[] fd_y0; delete[] fd_u; delete[] fd_dt;
            return -1;
        }
        
        getSensitivitiesY0(analytical_y0);
        getSensitivitiesU(analytical_u);
        getSensitivitiesDt(analytical_dt);
        
        // Compute finite difference sensitivities
        cout << "Computing finite difference sensitivities..." << endl;
        result = computeFullSensitivityFiniteDifference(initial_states, torque_params, delta_t,
                                                       epsilon, fd_y0, fd_u, fd_dt);
        if (result != 0) {
            cerr << "Error: Finite difference computation failed" << endl;
            delete[] analytical_y0; delete[] analytical_u; delete[] analytical_dt;
            delete[] fd_y0; delete[] fd_u; delete[] fd_dt;
            return -1;
        }
        ...
        vector<sunrealtype> y0_errors, u_errors, dt_errors;
        
        for (int i = 0; i < y0_nnz; i++) {
            y0_errors.push_back(abs(analytical_y0[i] - fd_y0[i]));
        }
        
        for (int i = 0; i < u_nnz; i++) {
            u_errors.push_back(abs(analytical_u[i] - fd_u[i]));
        }
        
        for (int i = 0; i < dt_nnz; i++) {
            dt_errors.push_back(abs(analytical_dt[i] - fd_dt[i]));
        }
        
        // Compute statistics
        auto y0_max = *max_element(y0_errors.begin(), y0_errors.end());
        auto y0_avg = accumulate(y0_errors.begin(), y0_errors.end(), 0.0) / y0_errors.size();
        
        auto u_max = *max_element(u_errors.begin(), u_errors.end());
        auto u_avg = accumulate(u_errors.begin(), u_errors.end(), 0.0) / u_errors.size();
        
        auto dt_max = *max_element(dt_errors.begin(), dt_errors.end());
        auto dt_avg = accumulate(dt_errors.begin(), dt_errors.end(), 0.0) / dt_errors.size();
        
        // Report results
        cout << "\n=== Comparison Results ===" << endl;
        cout << "∂y/∂y₀ errors: Max=" << scientific << y0_max << ", Avg=" << y0_avg << endl;
        cout << "∂y/∂u errors:  Max=" << scientific << u_max << ", Avg=" << u_avg << endl; 
        cout << "∂y/∂dt errors: Max=" << scientific << dt_max << ", Avg=" << dt_avg << endl;
        
        sunrealtype overall_max = max({y0_max, u_max, dt_max});
        bool passed = overall_max < tolerance;
        
        cout << "Overall max error: " << scientific << overall_max << endl;
...
int main() {
    try {
        ...
        sunrealtype epsilon = 1e-6;
        sunrealtype tolerance = 1e-4;

        int result = integrator.compareAnalyticalVsFiniteDifference(epsilon, tolerance);
